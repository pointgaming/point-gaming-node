...
var PointGaming = global.PointGaming,
    authenticator = require("./commands/authenticate"),
...
client.prototype.registerEventHandlers = function () {
    ...
    this.socket.on("idle", self.setIdle.bind(this));
    ...
    this.socket.on("friends", function () {
        self.listFriendsHandler();
    });
...
client.prototype.setIdle = function (message) {
    ...
    if (this.user) {
        this.setCurrentUserStatus(idle === 1 ? "idle" : "online");
    }
};
...
client.prototype.setCurrentUserStatus = function (new_status) {
    PointGaming.current_user_api_client.setUserSession(this.auth_data.auth_token, {
        status: new_status
    }, function (err, data) {});
    ...
    this.createExchange(name, function (err, exchange) {
        ...
        PointGaming.amqp_conn.queue("", {
            ...
            queue.subscribe(function (message) {
                messageCallback.call(self, name, message);
            });
        });
    });
};
