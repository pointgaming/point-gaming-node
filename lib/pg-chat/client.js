...
// Talk to Dean about this change before we implement it
//    sendMessage = require('./commands/message/send'),
    sendMessage = require('./commands/message'),
...
client.prototype.userExchangeMessageHandler = function(exchange_name, message){
  ...
  } else if (message.user_id && message.message) {
// TODO: talk to Dean about this change, then enable it
//    receiveMessage.handle.bind(receiveMessage, this);
    this.socket.emit('message', {user_id: message.user_id, message: message.message});
  }
};
...
client.prototype.messageHandler = function(data) {
  if (data.chat && data.message) {
    ...
    var message = {user_id: this.user._id, message: data.message};
    this.sendMessageToExchange(this.USER_EXCHANGE_PREFIX + data.user_id, message);
  }
};
