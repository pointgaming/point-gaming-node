var authenticator = require('./commands/authenticate'),
    ...
    SocketUser = require('./models/socket_user'),
...
client.prototype.setCurrentUser = function(user, authMessage) {
  ...
  socketUser.set(this.socket.id, this.user._id);
...
client.prototype.disconnectHandler = function(){
  ...
  socketUser.remove(this.socket.id);

  if (this.queues.length) {
    async.each(this.queues, function(item, callback){
      if (item.name.substr(0, 2) === self.CHAT_EXCHANGE_PREFIX) {
        self.unsubscribeFromExchange(item.name.substr(2));
        self.removeFromChat(item.name.substr(2));
      } else {
        item.queue.destroy();
      }
    }, 
