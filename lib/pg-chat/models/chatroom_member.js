...
Model.prototype.add = function(chat_name, username, points, callback) {
  PointGaming.redis_client.zadd(this.getKey(chat_name), points, username, 
      runCallbacksForEvent.bind(this, 'afterAdd', chat_name, callback));
};
...
Model.prototype.exists = function(chat_name, username, callback) {
  PointGaming.redis_client.zrank(this.getKey(chat_name), username, callback);
};

Model.prototype.remove = function(chat_name, username, callback) {
  PointGaming.redis_client.zrem(this.getKey(chat_name), username, 
      runCallbacksForEvent.bind(this, 'afterRemove', chat_name, callback));
};

var runCallbacksForEvent = function(event_name, chat_name, callback, err, return_value) {
  ...
  if (chat_type && eventCallbacks && eventCallbacks[event_name] && typeof(eventCallbacks[event_name][chat_type]) === 'function') {
    eventCallbacks[event_name][chat_type].bind(this)(chat_name, chat_id, callback, return_value);
  } else {
...
var eventCallbacks = {
  afterAdd: {
    ...
    Stream: function(chat_name, chat_id, callback, return_value) {
      if (return_value === 1) {
  ...
  afterRemove: {
    ...
    GameRoom: function(chat_name, chat_id, callback, return_value) {
