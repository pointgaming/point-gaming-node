...
ChatroomMember.prototype.add = function(chat_name, user_id, callback) {
  PointGaming.redis_client.sadd(this.getKey(chat_name), user_id, 
...
ChatroomMember.prototype.all = function(chat_name, callback) {
  PointGaming.redis_client.smembers(this.getKey(chat_name), callback);
};

ChatroomMember.prototype.count = function(chat_name, callback) {
  PointGaming.redis_client.scard(this.getKey(chat_name), callback);
};

ChatroomMember.prototype.exists = function(chat_name, user_id, callback) {
  PointGaming.redis_client.sismember(this.getKey(chat_name), user_id, callback);
};

ChatroomMember.prototype.remove = function(chat_name, user_id, callback) {
  PointGaming.redis_client.srem(this.getKey(chat_name), user_id, 
...
var eventCallbacks = {
  afterAdd: {
    ...
    Game: function(chat_name, user_id, chat_id, callback, return_value) {
      if (return_value === 1) {
        LobbyApiClient.join(chat_id, user_id);
      }
      callback(null, return_value);
    },
  ...
  afterRemove: {
    ...
    Game: function(chat_name, user_id, chat_id, callback, return_value) {
      LobbyApiClient.leave(chat_id, user_id).on('complete', function(result){
        callback(null, return_value);
      });
    },
