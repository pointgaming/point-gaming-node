...
Command.prototype.handle = function(socket, message) {
  if (message._id) {
    // TODO: determine if I need to destroy the initial handler
    socket.unsubscribeFromExchange(socket.CHAT_EXCHANGE_PREFIX + message._id);

    socket.emit('leave_chat', {_id: message._id});
  }
};
