  ...
  PointGaming.models.ChatroomMember.count(message._id, function(err, count) {
    if (err) callback(err);
      ...
      async.parallel({
        ...
        if (err) callback(err);
        else if (!results.isInvitedToChat && results.isCurrentlyInChat === null) callback(new Error('Permission Denied.'));
        else {
...
var joinChat = function(socket, message) {
  ...
  PointGaming.models.ChatroomMember.add(message._id, socket.user._id, socket.user.points || 0);
...
var handleError = function(err, socket, message) {
  socket.emit('join_chat', {success: false, message: 'There was an error when joining the chat.'});
};
